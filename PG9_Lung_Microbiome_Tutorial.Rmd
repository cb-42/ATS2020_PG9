...
R Markdown documents consist of narrative of text such as this and code chunks such as the one below. Code chunks are enclosed within two pairs of three backticks (for reference a backtick is denoted by `).  
...
# The first line without a # symbol is R code, and it will be executed when this code chunk is run.
# To execute an entire code chunk: green arrow in upper right of this section:
# Alternatively, place your cursor on the line with R code, then use: cmd + enter (mac), ctrl + enter (pc)
...
otu_good[1:20, 1:10] # use other means of showing a glimpse of the data?
...
ggplot(avg_abs_df, aes(x = Type, y = mean_DNA_copies, fill = Type)) +
  ...
  scale_y_log10() + # We could convert to log10 scale prior to plotting, or we can do it on the fly
  ...
  theme_bw() +
  labs(x = "Specimen or Control Type", y = "log10 mean_DNA_copies")
```
