...
Here is a glimpse of the metadata:
...
Cage example using `vegan::cca()`. [Descriptive text here]  
...
fam_df <- otu_df %>%
  ...
  tidyr::gather(key = OTU, value = Percentage, -stringr::str_which(colnames(.), "Otu", negate = TRUE)) %>% # Take data into long form in preparation for join operation
...
ggsave(filename = "testplot.pdf", plot = ra_vend_plot, width = 30, height = 30, units = "cm")
```
...
Note that there will also be a figure-html folder containing all of the plots created through the code chunks in this document. For example, if the Rmd file (the one we're using) is called PG9_Lung_Microbiome_Tutorial, then the figures would be located in: PG9_Lung_Microbiome_Tutorial_files/figure-html.  
