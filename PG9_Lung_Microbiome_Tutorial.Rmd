...
otu_order <- names(sort(colMeans(filter(ra_df, tissue_type == "Lung")[, stringr::str_detect(colnames(ra_df), "Otu")]), decreasing = TRUE))
...
levels(agg_df$OTU) <- paste_tax(levels(agg_df$OTU))
# equivalent to:
# fct_inorder(paste0(otu_good_taxonomy[levels(agg_df$OTU), ]$Genus, " (", levels(agg_df$OTU), ")"))
...
# otu_good_spc_pca <- rda(otu_good_spc_hel)
# 
