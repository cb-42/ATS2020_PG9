...
# I. Introduction
Welcome to the hands-on introduction to studying the lung microbiome. In this session we'll see how we can use R to perform data manipulation, modeling, visualization, and reporting as part of our analysis workflow. If you have not already done so, please download R, RStudio, and the necessary packages. See this [guide](https://github.com/cb-42/ATS2020_PG9) for more details.
...
## 1.4 Brief notes on R code syntax, best practices  
  ...
  - Give your objects meaningful, formulaic names. This is not always easy when you create dozens or hundreds of variables.  
...
otu_raw <- read.table("data/Heterogeneity.shared.txt", row.names = 2, header = TRUE) # read in the data, treating the first row as column names
...
otu_good_taxonomy[1:10,] # Examine the first 10 rows. These 10 OTUs coincide with the glimpse we took of otu_good.
```
...
spec_abs <- readxl::read_xlsx("data/16S_EvaGreen__Heterogeneity1_Data.xlsx", sheet = 2) %>%
  ...
  dplyr::filter(str_detect(Sample_name, "LUNG"))
...
abs_df <- full_join(spec_abs, ctrl_abs, by = c("Sample_name", "DNA_copies_per_ul")) %>% # We could also use bind_rows() or rbind() in this scenario since we renamed the columns to match. However, for more complex operations, it's safer to use a join function and specify the key columns.
  mutate(Type = factor(case_when( # We need to create a factor containing the types of samples for creating separate bars in an upcoming visualization.
    str_detect(Sample_name, "LUNG") ~ "Lung", 
    TRUE ~ str_extract(Sample_name, "^[:alpha:]+")
  ), levels = c("AE", "HomogCtrl", "IsoCtrl", "NTC", "Lung")))

# We also need to compute the averages and error
avg_abs_df <- abs_df %>%
  group_by(Type) %>%
  summarize(mean_DNA_copies = mean(DNA_copies_per_ul), SEM = sqrt(var(DNA_copies_per_ul)/length(DNA_copies_per_ul))) %>%
  ungroup()
```

```{r abs_abundance_plot, fig.height = 8, fig.width = 12}
ggplot(avg_abs_df, aes(x = Type, y = mean_DNA_copies, fill = Type)) +
  geom_col() + 
  geom_errorbar(aes(ymin = mean_DNA_copies - SEM, ymax = mean_DNA_copies + SEM, width = 0.5)) +
  ...
  geom_jitter(data = abs_df, aes(y = DNA_copies_per_ul), shape = 1, size = 2) + # shape 1 is an unfilled circle
  theme_bw() +
  labs(x = "Control or Specimen Type", y = "log10 mean_DNA_copies per µL", title = "Absolute abundance of negative controls and lung specimens") +
...
[PCA Comparison of negative controls vs lung tissue]  
[Descriptive text here]  
...
rownames(ra_df) <- ra_df$specimen

# PCA of all negative controls + lung tissue
otu_con_lung_hel <- decostand(dplyr::select(ra_df, contains("Otu")), "hellinger") # use OTU columns only
...
# Permanova
set.seed(8947) # setting a seed allows for reproducibility - both lines need to be run together
adonis(otu_con_lung_hel~ra_df$vendor, method="euclidean", permutations = 10000) # 9.999e-05 ***
```

```{r spider_con_lung, fig.height=10, fig.width=10}
# This approach combines base R plotting functionality and vegan's ordispider function, which creates the lines connecting points to the centroids.
plot(otu_con_lung_pca, type = "n", font = 2, font.lab = 2, xlab = "PC1 (17.82% Explained)", ylab = "PC2 (8.60% Explained)", main = "PCA of Specimen Type, colored by Type", display = "sites")
points(otu_con_lung_pca, pch = 19, col = pcapal[as.numeric(ra_df$reagent_tissue)])
ordispider(otu_con_lung_pca, ra_df$reagent_tissue, label = TRUE)
legend("topright", levels(ra_df$reagent_tissue), pch = 19, col = pcapal, title = "Type")
legend("topleft", legend = "Permanova: Difference between Specimen Type: 9.999e-05") # Should signifiance test be include in the plot?
```

[biplot descriptive text]

```{r biplot, fig.height=10, fig.width=10, warning=F}
...
# Alternatively, set warning = F in the code chunk
...
biplot(prcomp(otu_con_lung_hel), xlabs=ifelse(str_detect(ra_df$reagent_tissue, "Control"), "C", "T")) # arrow.len = 0 will also remove warnings, at the cost of losing the arrowhead
```
...
# Order the data by most abundant OTU in lung tissue specimens
# Include an errorbar
# Color by phyla
...
ra_df <- otu_df %>% # Note at this is just one of several possible approaches to filtering to retain the correct rows
  filter(tissue_type == "Lung") %>% # 65 specimens
  droplevels() # remove factor levels with no remaining observations - saves potential headaches in downstream wrangling and visualizations
...
```{r ra_lung, fig.height=8, fig.width=12}
...
Here is an example of exporting `otu_lung` to a .csv file, which can then be opened in Excel or your spreadsheet editor of choice. Note that depending on your locale, you may need to use `write.csv2()` to obtain the desired results. As an additional exercise, try changing the name of the dataframe or the name of the output file.
...
ggsave(filename = "testplot.png", plot = p, width = 12, height = 10, units = "cm")
```
...
Note that there will also be figure-html folder containing all of the plots created through the code chunks in this document. For example, if the Rmd file (the one we're using) is called PG9_Lung_Microbiome_Tutorial, then the figures would be located in: PG9_Lung_Microbiome_Tutorial_files/figure-html.  
