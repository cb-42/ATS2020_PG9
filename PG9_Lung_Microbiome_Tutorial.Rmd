...
Example using vegan. [descriptive text here]
...
# label OTUS
otu_ven_sigpvals$OTU <- paste_tax(otu_ven_sigpvals$OTU)
```
...
a <- Sys.time()
set.seed(3462)
rf_ven <- randomForest(vendor ~ ., data = rf_df, importance = T)
varImpPlot(rf_ven, scale = F)
b <- Sys.time()
b-a
...
Let's run the random forest model 100 times (note: this took around 30 seconds on a powerful machine).
...
for (i in seq_along(1:nforest)) {
    rf_list[[i]] <- randomForest(vendor ~ ., data = rf_df, importance = T)
}
b <- Sys.time()
b-a
...
levels(fi_df$Feature) <- paste_tax(levels(fi_df$Feature))
```

Next we'll create boxplots of the feature importance over the 100 random forest runs.

```{r rf_feature_importance, fig.height=10, fig.width=10, cache = TRUE}
