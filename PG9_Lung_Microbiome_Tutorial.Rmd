...
We will explore PCA ordinations, biplots, rank abundance plots (bar charts), heat maps, 
...
rownames(con_lung_df) <- ra_df$specimen 
...
adonis(otu_con_lung_hel~con_lung_df$vendor, method="euclidean", permutations = 10000) # 9.999e-05 ***
```
...
legend("topleft", legend = "Permanova: Difference between Lung Specimens and Controls: 9.999e-05") # Should signifiance test be include in the plot?
```
...
heat_df <- otu_lung %>% 
  ...
  gather(key = OTU, value = Percentage, -log_IL1a, -specimen, factor_key = TRUE) #%>% # Reshape the dataframe from wide to long
  # group_by(OTU) %>%
